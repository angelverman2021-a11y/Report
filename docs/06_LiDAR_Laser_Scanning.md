...
| **Volumetric ($m^3$) Calculation**| **Direct & Exact** | ❌ Impossible | ❌ Coarse estimation only | ❌ Coarse estimation only |
| **Structural Joint Extraction** | **Direct Automated Extraction** | ❌ Impossible | ❌ Impossible | ❌ Impossible |
...
graph TD
    subgraph Explainable Alert Notification Card
        A["🔴 LEVEL 4: CRITICAL ROCKFALL ALERT (Risk Probability: 94.2%)"]
        B["📍 Location: Bench 3 - North Wall (Grid Sector NW-12)"]
        C["⏱️ Predicted Collapse Window: 24 ± 5 minutes"]
...
| **Hydrogeological Awareness** | ❌ Blind to subsurface conditions | **✅ Synchronized Vibrating-Wire Piezometer Telemetry** |
