# MINE-SAFE AI: AI-Based Rockfall Risk Prediction & Early-Warning System for Open-Pit Mines
...
Open-pit mining accounts for a major portion of mineral and coal extraction in India. As surface mines excavate deeper into complex geological formations, **rockfalls, bench shear failures, and highwall instability** represent severe hazards to personnel and heavy equipment (such as electric rope shovels, excavators, and haul trucks).

### Regulatory & Operational Drivers (DGMS Guidelines):
Under **DGMS (Tech) Circular No. 02 of 2020** and the **Coal Mines Regulations (CMR 2017)**, open-cast mines are statutorily required to maintain scientific slope monitoring and execute **Trigger Action Response Plans (TARP)**. However, conventional monitoring approaches encounter significant operational challenges:

```
+---------------------------------------------------------------------------------------------------+
|                        CHALLENGES IN CONVENTIONAL MINE SLOPE MONITORING                           |
+---------------------------------------------------------------------------------------------------+
|  1. HIGH CAPITAL COST: Commercial Slope Stability Radar (SSR) systems require significant capital  |
|     expenditure, limiting continuous radar deployment across many small and medium-sized pits.    |
|  2. SPATIAL POINT BLINDNESS: Discrete in-situ sensors (prisms, crackmeters) monitor isolated      |
|     locations; localized instability developing between sensor points may go undetected.         |
|  3. DATA ISOLATION: Radar, piezometers, weather stations, and total stations often operate on      |
|     disconnected vendor platforms, creating delays in data consolidation during emergency response|
|  4. STATIC THRESHOLDS: Fixed displacement thresholds can lead to elevated false-alarm rates during|
|     benign creep or fail to capture rapid non-linear failure trends.                              |
+---------------------------------------------------------------------------------------------------+
```
...
| *3D Terrain Reality Mesh with Zone Risk Overlays* | *Optical Flow Tracking & Bounding Box Detection (Illustrative)* |
...
| *Solar PTZ Camera, LoRa Mast, Weather Sensor & Siren* | *Low-Cost Crest Crackmeter & MEMS Sensor Node Prototype* |

---

## 4. The Proposed Solution: MINE-SAFE AI

**MINE-SAFE AI** integrates multi-source monitoring data around the core concept:

$$\mathbf{Risk} \times \mathbf{Location} \times \mathbf{Time}$$

Instead of displaying disconnected raw numbers, the system processes heterogeneous telemetry to deliver **zone-level spatial risk intelligence**:

```
Monitoring Data (Heterogeneous Sources)
      ↓
Data Cleaning, Validation & Spatiotemporal Synchronization
      ↓
Physics-Informed Feature Engineering (Velocity, Acceleration, Pore Pressure Ratio)
      ↓
AI Risk Estimation & Classification (Safe / Moderate / High / Critical)
      ↓
Kinematic Risk Velocity & Estimated Forecast Horizon
      ↓
Explainable AI (SHAP Causal Factor Breakdown)
      ↓
3D Spatial Visualization / Digital Twin Prototype
      ↓
Dynamic TARP Engine & Early-Warning Alerts
      ↓
Human-in-the-Loop Verification & Authorized Action
```
...
| **04** | [**`04_TECHNOLOGY_COMPARISON.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/04_TECHNOLOGY_COMPARISON.md) | Multi-dimensional benchmark comparison across coverage, sampling rate, physics, and complexity. |
...
| **08** | [**`08_SYSTEM_ARCHITECTURE.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/08_SYSTEM_ARCHITECTURE.md) | 5-tier decoupled software architecture and prototype hardware Bill of Materials. |
...
| **10** | [**`10_AI_ML_ARCHITECTURE.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/10_AI_ML_ARCHITECTURE.md) | Core tabular ML (XGBoost), LSTM trend forecaster, Saito IV model, and SHAP XAI. |
| **11** | [**`11_SENSOR_FUSION.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/11_SENSOR_FUSION.md) | Heterogeneous sampling rate alignment and 2D-to-3D optical flow projection. |
...
| **14** | [**`14_EARLY_WARNING_TARP.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/14_EARLY_WARNING_TARP.md) | 4-tier TARP engine, multi-channel alerting, and Human-in-the-Loop governance. |
...
| **17** | [**`17_SECURITY_AND_RELIABILITY.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/17_SECURITY_AND_RELIABILITY.md) | Fail-safe store-and-forward buffering, dual-path networks, RBAC, and audit logs. |
...
> **Narrative Statement:**  
> *"The 26 technologies represent the broad research landscape. A selected subset forms the SIH prototype, while capital-prohibitive or specialized industrial systems remain as researched background data inputs and future industrial extensions."*
...
| :---: | :--- | :--- | :---: | :--- |
| **01** | **Slope Stability Radar (SSR)** | [**`01_SSR.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/technologies/01_Slope_Stability_Radar_SSR.md) | `[EXISTING]` `[RESEARCHED]` | Saito inverse velocity math adopted in AI; physical hardware in future scope. |
...
| **03** | **Satellite InSAR (SBAS)** | [**`03_Sat_InSAR.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/technologies/03_Satellite_InSAR_DInSAR_PSInSAR_SBAS.md) | `[EXISTING]` `[DATA INPUT]` | Copernicus Sentinel-1 SBAS subsidence velocity maps ingested via API. |
...
| **13** | **Tiltmeters (MEMS)** | [**`13_Tiltmeters.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/technologies/13_Tilt_Sensors_Tiltmeters.md) | `[EXISTING]` `[PROTOTYPE]` | Custom low-cost LoRa MEMS tilt nodes with Kalman filter. |
...
| **19** | **CCTV Fixed Cameras** | [**`19_CCTV_Cameras.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/technologies/19_CCTV_Fixed_Cameras.md) | `[EXISTING]` `[PROTOTYPE]` | Upgrades existing mine CCTV cameras via edge RTSP streaming. |
...
| **25** | **Digital Twin 3D** | [**`25_Digital_Twin.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/technologies/25_Digital_Twin_3D_Mine_Monitoring.md) | `[EXISTING]` `[PROTOTYPE]` | Interactive 3D WebGPU browser Digital Twin prototype with Zone Intel. |
| **26** | **Early-Warning TARP** | [**`26_TARP_Systems.md`**](file:///Users/angle/.gemini/antigravity/scratch/SIH25071-Rockfall-Prediction-System/docs/technologies/26_Early_Warning_TARP_Systems.md) | `[EXISTING]` `[PROTOTYPE]` | Automated 4-tier TARP dispatcher for sirens, VHF radio & SMS alerts. |
...
4. **Future Risk Forecast `[PROTOTYPE]`:** Time-series neural forecasting coupled with the physical **Saito Inverse Velocity Model** ($\text{IV} = 1/v \to 0$) to estimate risk trends and failure-risk indicators ($t_f \pm \sigma$).
5. **Computer Vision Evidence `[PROTOTYPE]`:** Optical camera analytics providing visual evidence through sub-pixel Lucas-Kanade optical flow, DeepCrack tension crack segmentation, and YOLOv8 falling boulder tracking.
6. **Explainable Risk (SHAP Causal Attribution) `[PROTOTYPE]`:** Local Shapley value decomposition explaining contributing physical drivers behind elevated risk scores (e.g., creep acceleration + pore pressure + crack opening + rain infiltration).
7. **Prediction Confidence Index ($\mathbf{C_{\text{pred}}}$) `[PROTOTYPE]`:** Reliability metric based on sensor density, telemetry freshness, packet delivery ratio ($PDR$), and model uncertainty.
...
9. **Smart Alerts & Dynamic TARP `[PROTOTYPE]`:** Multi-tier Trigger Action Response Plan with automated multi-channel dispatch across pit sirens, two-way VHF emergency radio, and SMS.
